...
// Close session writing
	...
	include( "{$_SERVER['DOCUMENT_ROOT']}/includes/html/appointments/comselectproperty.php" );
	...
	if ( $result[ $get_property->id ][ 'status' ] ) {
		...
		$selectproperty_form->add_element( new Element( 'hidden', [
			'label' => 'appjcustomer_id',
			'name'  => 'appcustomer_id',
			'value' => ( isset( $_POST['appcustomer_id'] ) ? $_POST['appcustomer_id'] : '' ),
			'class' => 'width-300px label-required'
		] ) );
		...
		$selectproperty_form->add_element( new Element( 'hidden', [
			...
			'value' => ( isset( $_POST['appcustomer_name'] ) ? $_POST['appcustomer_name'] : '' ),
			'class' => 'width-300px label-required'
		] ) );
