<?php namespace property;
	...
	// Verify hash
		...
		$property_query = <<<SQL
SELECT `property`.property_id, `property`.folder_id, `property`.name,`property`.address1,`property`.address2,`property`.city,`state`.state_id,`state`.state,`property`.zip,`property`.type_id
  FROM `property`
		INNER JOIN `state` 
				ON `state`.state_id=`property`.state_id 
        $query_where
        $query_order
		$query_limit
SQL;
		...
		$data = array();
			foreach ( $property_row = $property_stmt->fetchAll( \PDO::FETCH_ASSOC ) as $i => $property_row ) {
				$data[ $i ] = $property_row;
			}
