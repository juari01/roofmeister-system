<?php namespace property;
	...
	function delete_linkcustomer( $params ) {
	/**
	...
	$customer_id = $params['xrefcustomer_id'];
	$property_id = $params['xrefproperty_id'];

		$del_link_customer_query = <<<SQL
  DELETE
  	FROM xref_customer_property
   WHERE customer_id = :customer_id
   	 AND property_id = :property_id
SQL;
	$del_link_customer_stmt = \DB::dbh()->prepare( $del_link_customer_query );
	$del_link_customer_stmt ->execute( array(
		':customer_id' => $customer_id,
		':property_id' => $property_id
	));
		

		return \JSONRPC::build_result( TRUE, 'xref_customer_property' );
