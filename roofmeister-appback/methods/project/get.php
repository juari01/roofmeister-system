<?php namespace project;
	...
	// Assemble WHERE clause
		...
		$project_query = <<<SQL
SELECT `project`.project_id, `project`.folder_id, `customer`.customer_id,`property`.property_id, `customer`.name AS customer_name, `property`.name AS property_name, `project`.name AS project_name, `project`.description
  FROM `project`
		INNER JOIN `customer` 
				ON `customer`.customer_id=`project`.customer_id
		INNER JOIN `property` 
				ON `property`.property_id=`project`.property_id
         $query_where
         $query_order
		 $query_limit
SQL;
		...
		$data = array();
			foreach ( $project_rows = $project_stmt->fetchAll( \PDO::FETCH_ASSOC ) as $i => $project_rows ) {
				$data[ $i ] = $project_rows;
			}
...
SQL;
		...
		$project_count_stmt = \DB::dbh()->prepare( $project_count_query );
		\DB::bind_params( $project_count_stmt, $bind_params );
		$project_count_stmt->execute();
		
		$count = $project_count_stmt->rowCount();
