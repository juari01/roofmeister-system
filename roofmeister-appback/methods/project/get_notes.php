<?php namespace project;
	...
	// Assemble WHERE clause
		...
		if ( !empty( $params['project_id'] )) {
			$wheres[]                    = ' `xref_note_project`.project_id = :project_id';
		...
		$notes_query = <<<SQL
	select `note`.note_id,`note`.created,`note`.updated,`note`.note,`note`.is_system , `xref_note_project`.project_id, `user`.user_id , `user`.first_name
	  from `note`
INNER JOIN `xref_note_project`
    	ON `xref_note_project`.note_id = `note`.note_id
INNER JOIN `user`
