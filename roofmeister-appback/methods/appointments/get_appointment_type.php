<?php namespace appointments;
	...
	function get_appointment_type( $params ) {
	/**
	...
	// Assemble WHERE clause
		...
		if ( !empty( $params['type_id'] )) {
			$wheres[]                     = '`appointment_type`.type_id  = :type_id';
			$bind_params[':type_id'] = [
		...
		$query_where = !empty( $wheres )          ? ' WHERE ' . implode( ' AND ', $wheres ) : '';
	 	$query_order = !empty( $params['order'] ) ? ' ORDER BY ' . $params['order']         : 'ORDER BY type_id ASC';
		
		$appointmenttype_query = <<<SQL
		SELECT * FROM `appointment_type`
		$query_where
		$query_order
SQL;

		$appointment_stmt = \DB::dbh()->prepare( $appointmenttype_query );

		\DB::bind_params( $appointment_stmt, $bind_params );

		$appointment_stmt->execute();

		$appointment_row = $appointment_stmt->fetchAll( \PDO::FETCH_ASSOC );

		return \JSONRPC::build_result( TRUE, 'appointment_type', $appointment_row );
