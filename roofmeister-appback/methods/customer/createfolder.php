<?php namespace customer;
	...
	// Verify hash
		...
		$get_path_folder_id = \folder::get_folder_id_by_path('Customer');
		$pathfolderid       = $get_path_folder_id['folder_id'];
		$customernameid   	="$customername ($customer_id)";

//CHECK CUSTOMER FOLDER_ID IF NOT EMPTY 
...
//IF FOLDER_ID IS EMPTY THEN INSERT	
	$path_folder_id_query = <<<SQL
INSERT INTO `file_folder`
   SET parent_id = :parent_id,
name  = :customernameid
SQL;

		$path_folder_id_stmt = \DB::dbh()->prepare( $path_folder_id_query );
		$path_folder_id_stmt->bindParam( ':customernameid',$customernameid,\PDO::PARAM_STR );
		$path_folder_id_stmt->bindParam( ':parent_id',$pathfolderid,\PDO::PARAM_INT );
		$path_folder_id_stmt->execute();
...
SQL;
		...
		$last_id_row = $last_insert_id_row['last_id'];

//UPDATE CUSTOMER SET `FOLDER_ID` TO LAST INSERT ID
	$customer_query = <<<SQL
UPDATE `customer`
