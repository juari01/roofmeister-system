<?php namespace customer;
	...
	// Assemble WHERE clause
		...
		if ( !empty( $params['customer_id'] )) {
			$wheres[]                    = ' `xref_note_customer`.customer_id = :customer_id';
		...
		$notes_query = <<<SQL
	select `note`.note_id,`note`.created,`note`.updated,`note`.note,`note`.is_system , `xref_note_customer`.customer_id, `user`.user_id , `user`.first_name
	  from `note`
INNER JOIN `xref_note_customer`
    	ON `xref_note_customer`.note_id = `note`.note_id
INNER JOIN `user`
