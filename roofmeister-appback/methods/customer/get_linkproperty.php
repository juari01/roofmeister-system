<?php namespace customer;
	...
	function get_linkproperty( $params ) {
	/**
	...
	// Assemble WHERE clause
		...
		if ( !empty( $params['linkcustomer_id'] )) {
			$wheres[]                     = '`xref_customer_property`.customer_id  = :linkcustomer_id';
		...
		$property_query = <<<SQL
SELECT `xref_customer_property`.customer_id,`xref_customer_property`.`property_id`,`state`.state_id,`property`.name,`property`.address1,`property`.address2,`property`.city,`state`.state,`property`.zip
...
SQL;
		...
		return \JSONRPC::build_result( TRUE, 'property', $property_rows );
