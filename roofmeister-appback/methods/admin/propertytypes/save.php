<?php namespace admin\propertytypes;
	...
	// Validate submitted data
		...
		// Validate duplicate propertytypes
			$dupe_params = [
				'id'    => [ 'type_id' => ( isset( $params['where']['type_id'] ) ? $params['where']['type_id'] : 0 ) ],
				'field' => [ 'name'         => $params['values']['name'] ],
			];

			$dupe_count = \DB::check_duplicate( 'property_type', $dupe_params );

		// Return error and propertytypes if propertytypes has been used already
			if ( $dupe_count ) {
				return \JSONRPC::build_result( FALSE, 'property type has already been taken', $params['values']['name'] );
			}
		...
		// Begin transaction and commit only if all queries succeed
			...
			// Save info to database
				$property_type_id = \DB::save_to_db( 'property_type', $params );

				\DB::dbh()->commit();

				return \JSONRPC::build_result( TRUE, 'save_success', [ 'type_id' => $property_type_id ] );
