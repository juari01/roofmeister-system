<?php namespace admin\appointmenttypes;
	...
	// Specify the criteria
		$criteria = [
			...
			'required_insert' => [
				'calendar_id' => 'Missing Calendar Name',
				'name'		  => 'Missing Name'
			],
			'required_update' => [
				'calendar_id' => 'Missing Calendar Name',
	...
	// Validate submitted data
		...
		// Validate duplicate geography
			...
			$dupe_count = \DB::check_duplicate( 'appointment_type', $dupe_params );

		// Return error and geography if geography has been used already
			if ( $dupe_count ) {
				return \JSONRPC::build_result( FALSE, 'Appointment type name has already been taken', $params['values']['name'] );
			}
		...
		// Begin transaction and commit only if all queries succeed
			...
			// Save info to database
				$appointment_type_id = \DB::save_to_db( 'appointment_type', $params );

				\DB::dbh()->commit();

				return \JSONRPC::build_result( TRUE, 'save_success', [ 'type_id' => $appointment_type_id ] );
